...
# This path will also include play_index.txt and play_history.txt.
BASE_PATH = "/media/videos/"

# Video files, including subdirectories.
MEDIA_PLAYLIST = ['video1.mp4','video2.mp4','Series/E01.mp4']

# Number of videos to keep in history log, saved in play_history.text in
...
def write_schedule(file_list,previous_file = None):
    ...
    for filename in file_list:
        ...
        coming_up_next.append((next_time,"<strong>{}</strong>".format(filename)))
    ...
    html_contents = f"""<script src="https://cdn.jsdelivr.net/combine/npm/dayjs@1,npm/dayjs@1/plugin/timezone.min.js,npm/dayjs@1/plugin/utc.min.js,npm/dayjs@1/plugin/localizedFormat.min.js"></script>

                        <script>dayjs.extend(window.dayjs_plugin_utc);
                        ...
                        let imported_array = {js_array};

                        var times = new Array();

                        // Populate times with starting times and names of upcoming videos.
                        for (i of imported_array) {{
                            times.push(new dayjs.utc(i.time).utcOffset(offset).format('LT') + i.name + "<br />");
                        }}

                        </script>

                        Times are converted to your local time zone: <div id="timezone"></div>
                        <script>document.getElementById("timezone").innerHTML = timezone;</script><br /><br />
                        Now playing:<br />

                        <div id="nowplaying"></div><br />
                        <script>document.getElementById("nowplaying").innerHTML = times[0];</script>
                        
                        Coming up next:<br />
                        <div id="comingupnext"></div><script>document.getElementById("comingupnext").innerHTML = times.slice(1).join("");</script>
