...
if __name__ == "__main__":
    ...
    while True:
        if play_index < len(media_playlist):
            ...
            # TODO: Delay playback for several seconds to account for window 
            # capture delay.

            # If HTML schedule writing is enabled, retrieve next videos in 
            # list up to SCHEDULE_UPCOMING_LENGTH and write_schedule in second
            # process.
            if SCHEDULE_PATH != None:
                ...
                # If media_progress is shorter than SCHEDULE_UPCOMING_LENGTH, 
                # copyfull media playlist until the correct length is reached.
                else:
                    media_copy = (media_progress 
                                 + list(itertools.islice(
                                   itertools.cycle(media_playlist),
                                   SCHEDULE_UPCOMING_LENGTH
                                 - len(media_progress) + 1)))

                schedule_p = Process(target=write_schedule,args=(media_copy,),
                                     kwargs={"previous_file":
                                     media_playlist[play_index - 1]})
            ...
            # If scheduling is disabled, simply play files in single process.
            else:
