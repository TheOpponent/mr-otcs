...
def test_print2_functions(monkeypatch,capsys):
    ...
    assert capture.out == datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " " + "[Info]" + " Super Verbose message\n"

    print2("verbose","Verbose message")
    capture = capsys.readouterr()
    assert capture.out == datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " " + "[Info]" + " Verbose message\n"
    ...
    capture = capsys.readouterr()
    assert capture.out == datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " " + '\033[31m' + "[Error]" + '\033[0m' + " Fatal message\n"
